...
`teal_data`. It is build in a way filter panel recalculates data only
when filter-panel-state or loaded-data changes and when panel is
visible. Thanks to this special implementation all modules' data are
recalculated only for those modules which are currently displayed.

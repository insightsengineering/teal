...
This argument allows to pass a `list` of `teal_data_module` objects that are created either with `teal_transform_module()` or `teal_data_module()` functions.

The main benefit of `teal_transform_module()` is the ability to transform data before passing the 
data into the module. This feature allows to extend the regular behavior of existing modules by specifying custom data operations on data inside this module.

`teal_transform_module()` is a Shiny module that takes `ui` and `server` arguments. When provided, `teal` will execute data transformations for the specified module when it is loaded and each time the data is changed. `server` extend the logic behind data manipulations, where `ui` extends filter panel with new UI elements that orchestrate the transformer inputs.

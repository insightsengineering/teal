...
_Note_: It is recommended to return `reactive()` with `teal_data()` in `server` of `teal_transform_module` as this is more robust for maintaining the reactivity of Shiny.
If you are planning on using `eventReactive()` in the server it needs to be reactive on to `data()` _(example `eventReactive(list(input$a, data()), {...})`)_.

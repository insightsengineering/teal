...
Note down the key changes made to this module to make it decorate-friendly:
1. The `ui_args` and `server_args` arguments get the `decorators`.
2. The `decorators` object is consumed by the `ui_transform_teal_data` and `srv_transform_teal_data` in the `ui` and `server` functions, respectively.

...
the `asl` data with the `USUBJID` variable.

<div style="background: #ff6666; padding: 20px; color: white;">Note that currently the list names need to be lower case. This currently makes the API inconsistent. We will address this in the coming releases of teal.</div>
...
is the `tabs` arguments. Tabs define the elements in the *Navbar* and their 
corresponding content in the *Tabs Content* box. A `tabs` object can contain 
`tab_item` and `tabs_item` objects where the `tabs_item` contains a `tabs` slot 
in order to create a tree of page content for the web app. For the app defined
above this tree looks as follows:

<img src="images/tabs.png" alt="Navigation Tree" style="width: 100%;"/>

Note that we currently only allow a tree of depth 2, so the `tabs_item` object 
can not contain any other `tabs_item` objects in its `tabs` slot.

The [shiny modules][shiny_modules] are specified in the `tab_item` function with
the `server` and `ui` argument. Both, `tab_item` and `tabs_item` are documented 
in the `teal` package, see `?tab_item` and `?tabs_item`.
...
Assume we create the following `tab_item` with the shiny modules
...
specified in the `tab_item` function. The `server_args` expects a named list 

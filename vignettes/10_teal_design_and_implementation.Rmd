...
the the `server` function that provies an access to the filtered data.
...
app. `shiny` provides a reactivity environment to update and get information
from the ui:
...
`html` code from the previous example app (running) with Chrome's developer
tools and then stop the shiny app and compare that html code with the ui code
created by running the following code in the R console:
...
object and *server* function into resuable and self-contained functions. Please
...
`html` and reactivity names. Please inspect the `html` code for the above
example and pay attention to the `html` `id` attributes.
...
wide standard for creating and sharing shiny code and also for creating project
...
x <- teal::init(
  data = list(ASL = iris %>% mutate(USUBJID = 1:n(), STUDYID = "A")),
  modules = root_modules(
    module(
      label = "sample module",
      server = function(input, output, session, datasets) {
        output$scatterplot <- renderPlot({
          iris <- datasets$get_data("ASL", filtered = TRUE, reactive = TRUE)
...
shows how simple it is to create a module that has no dynamic encodings and will
be only interactive in terms of filtering the data which is provided by `teal`.
The example also highlights a current issue where we require a dataset with the
name `ASL` that has the variables `USUBJID` and `SUBJID`. We decribe the reason
and the future refactoring needed to overcome this later in this document.
...
These have been the three main challanges (in addition to code reproducability)
...
When filtering a dataset, say `ASL`, with `10` of its variables then the `10`
...
anlysis dataset that contains the desired logic which then in turn can be
combined via `&` logic with other variable based subsetting contitions.

### Clinical Trials Data

In order to understand the current implementation of the `FilteredData` class it
is usful to have some background of the data standards used in clinical trials.
...
an output can be computed directly from a single ADaM data set. Here is a diagram
of some SDTM datasets:

```{r echo=FALSE, out.width='80%'}
knitr::include_graphics('./images/sdtm_overview.png')
```

The ADaM datasets names often add a `AD` or `A` in front of their corresponding
`SDTM` dataset names.

There is a particularly important dataset called `ASL` or `ADLS` that contains
...
most outputs (e.g. `ARM`). The key for merging `ASL` and any other analysis
dataset is the `USUBJID` and `STUDYID` variable (if not specified otherwise).

### FilteredData for Clinical Trials Data

Because of the central role of the `ASL` dataset and due to tight project
delivery timelines I have unfortunately implemented a filtering mechanism that
always depends on `ASL` as illustrated here:

```{r echo=FALSE, out.width='100%'}
knitr::include_graphics('./images/design_cdisc_filtered_datasets.png')
```

This fitering will now be explained step by step for two example datasets (`ASL`
and `ATE`):
...
Two `options` calls are necessary due to a bug (i.e. `list_data_info` should not
use `.log` for listing the info). Now say we want to filter out all the patients
except patients with `USUBJID` equal to `id-1`, `id-2`, `id-3`, or `id-4`:
...
The two `get_data` method calls return the `ASL_FILTERED` and `ATE_FILTERED`
...
that is, filtering expression sets is a slighlty more advanced filtering as one
...
a propagation mechanism that happens to be inefficient. That is, if when
filtering with `ASL` variables then all the dependent datasets (that is
currently every other dataset) gets updated regardless if there is any ouptut
currently displayed that requires the information from a particular dataset.
This partly breaks the lazy evaluation that is provided with shiny's reactivity
framework. It would be better to set up the datasets reactive and only have them
apply the filtering and merging if there is actually something displayed in the
shiny app that requires the particular dataset.
...
### Summary of fitlering related issues
...
* make the filtering lazy by making better use of reativity (using
`reactiveValues()`).
...
The datasets passed to the `data` argumemnts get encapsulated within the `init`
function into a local instance of `FilteredData` (i.e. the `datasets` object).

Next, the `ui` object is created. The compisition of ui and server happens
...
in `modules` happens recursivley via the `create_ui` function. Note that the
...
There are multiple parts to create reproducibile code for the displayed outputs:

1) what librariers are used, and which version

2) data used and hashsum (currently on BCE the path is the identifier and the
hashsum in needed as the data may change over time)

3) subset call to get the filtered data

4) code to create the output

We started to streamline 1) and 2) with the `get_rcode_header` function. This
funciton needs to be improved but that is a simple exercise. Next, 3) can either
be achieved via the `get_filter_call` method of the `datasets` object or simpler
via the `get_filter_txt` function. Finally 4) is currently the most difficult to
achive well. We have tried to create the R call (in a string to preserve the
formatting) and then evaluating that string to create the output, however that
is not a good approach as it makes it essentially impossible to debug the
modules. Another approach that we have tried is to use tagging with
`@start_label` and `@end_label` as documented in the `02_reproducible-code`
vignette. Two more promising approaches are to tag the code chunks via a
function call with tag and chunk-code and then remove the function call but
leave the code when creating the server function (as proposed by Michael) and
another approach is developed by Doug and Gabe.


After having created a number of modules for
[`teal.oncology`](https://github.roche.com/Rpackages/teal.oncology) it seems
...
analysis package [tern](https://github.roche.com/Rpackages/tern) which has the
function `kmplot` to create a kaplan meier plot. We have then also created the
`teal.oncology` R package which has the module `tm_kplot` which provides a user
interface that allows one to modify certain arguments of `tern::kmplot` via the
encodings panel. This reduces 4) essentially to an argument tracking problem.
This is, however, work in progress.

## Desiderata

* save the state of an app and provide sharable links that capture the state of
the analysis/app
...
[tealGitHub]: https://github.roche.com/Rpackages/teal

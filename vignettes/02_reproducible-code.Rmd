...
module_spaghetti_chunks <- function(label = "Spaghetti Plot") {
...
ui_spaghetti_chunks <- function(id) {
  ...
  fluidRow(
      div(class = "well", plotOutput(ns("plot"))
    )
  )
...
x <- teal::init(
  ...
  tabs = tabs(
    ...
    module_spaghetti_chunks(label = "Spaghetti Plot") # Spaghetti Plot module
    )
  )
...
```{r, eval = F}
module_spaghetti_chunks <- function(label) {
...
the button. Note the use of `output_header` (explained at the end of this
...
observeEvent(input$show_rcode, {
  ...
  # Header
  str_head <- output_header(title = "Demo Spaghetti Plot",
...
The `output_header` function takes in the title, description (e.g. link to the 
...
If the datasets used are not from *"/opt/BIOSTAT/..."*, then `output_header` 
...
cat(output_header(title = "Demo Spaghetti Plot",
                  description = "Demo used for the walkthrough on building a teal Shiny module with reproducible code",
                  libraries = c("ggplot2", "dplyr", "haven"),
                  git_repo = "https://github.roche.com/Rpackages/teal",
                  data = c("ASL <- generate_sample_data('ASL', N = 1000)",
                           "ARS <- generate_sample_data('ARS', N = 1000)")))

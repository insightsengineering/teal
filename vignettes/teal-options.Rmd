...
* `"true"` - compute `lockfile` and show `"download lockfile"` in the footer
* `"false"` - do not compute `lockfile` and do not show `"download lockfile"` in the footer
* `"user"` - use pre-computed `lockfile` specified in `teal.lockfile.path` and show `"download lockfile"` in the footer
...
Used when `teal.lockfile.enable` is set to `"user"`. 
If files does not exist, the warning is thrown and `"download lockfile"` in the footer is not shown.

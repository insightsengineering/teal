...
We offer an easy application of a custom bootstrap theme to a `teal::init` app.
...
We like the Minty theme so we select this in the "Overall theme" drop-down.
...
Our base theme of Minty is close to what we want but let's make a few modifications.
...
In our `customizer` theme, we can go to "Options" and then uncheck the "Rounded corners" box.

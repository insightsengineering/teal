...
The `bslib::bs_theme` object creates a Bootstrap theme object, where you should to specify the (major) Bootstrap version (default or one of 3, 4 or 5).
...
When you have already a shiny app and want to test different bootstrap themes (and css styling) then `bslib::run_with_themer(shinyApp(app$ui, app$server))` is recommended.
...
One reason the theme is not update could be the problem that a browser caches the previous one, especially when we run different themes one after another.
...
Apply already chosen theme with `options("teal.bs_theme" = bslib::bs_theme(...))`.

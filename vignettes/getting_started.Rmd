...
We start the tree with the `root_modules` function which is a wrapper around 
`modules` that sets the `label` argument to `"root"`.
...
ADRS_FILTERED <- dplyr::inner_join(x = ADSL_FILTERED,
...
[teal_git]: https://github.roche.com/NEST/teal "Teal GitHub Repository"

...
More information can be found on the [agile-R website][agileR].
...
app <- teal::init(
  data =  list(ADSL = ADSL, ADRS = ADRS, ADTTE = ADTTE),
...
This will produce the following web app (I switched to the data table tab):
...
is based on the [R6][R6] object system and we initially did not intend to expose 
...
unfiltered dataset of interest.
...
module(
  ...
  server = function(input, output, session, datasets) {
    output$freq_table <- renderTable({
      adsl <- datasets$get_data("adsl", reactive = TRUE, filtered=TRUE)
      table(adsl$SEX)
    })
  }
)
...
trials data and that requires an `ADSL` analysis dataset to be part of every 
...
all datasets other then `ADSL` will also be filtered with the remaining subjects
...
```{r, eval=FALSE}
ADSL_FILTERED <- subset(ADSL, ADSL.STUDYID %in% c('BP29392', 'WP29945') & ADSL.AGE > 15 & ADSL.AGE < 35)
ADRS_FILTERED_ALONE <- subset(ADRS, ADRS.ADY > 0 & ADRS.ADY < 500)

ADRS_FILTERED <- merge(x = ADSL_FILTERED,
...
[agileR]: http://go.roche.com/agile-R

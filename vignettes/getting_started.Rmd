...
then load that `teal` package explicitly. To run the following code (say from
the [RStudio on bee](http://r.roche.com))

```{r, eval=FALSE}

project.path <- "/srv/shiny-server/<path>"
...
.libPaths(c(path.teal.libs, .libPaths()))
...
.libPaths(orig.libpaths)
setwd(current.path)
```

Then in you `app.R` add the following line as the first line:

```{r, eval = FALSE}
.libPaths(c(normalizePath("./libs"), .libPaths()))
```

The above line needs to be loaded before you load the `teal` library with
`library(teal)`.

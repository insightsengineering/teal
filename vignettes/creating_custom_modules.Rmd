...
srv_histogram_example <- function(id, datasets, histogram_var) {
  moduleServer(id, function(input, output, session) {
    ...
    plot_code <- reactive({
      ...
      # the name of the filtered data is <<dataname>>_FILTERED
      new_env <- new.env()
      assign(paste0(dataname(), "_FILTERED"), datasets$get_data(dataname(), filtered = TRUE), envir = new_env)
      teal.code::chunks_reset(envir = new_env)

      # add the code for the plot into the chunks
      teal.code::chunks_push(
        bquote(hist(.(as.name(paste0(dataname(), "_FILTERED")))[, .(selected())]))
      )
...
Then inputs are visible in the first reactive cycle.
A developer has to be aware of this situation as often It will be needed to use `shiny::req` or `ignoreInit` argument.
This side effect comes from the fact that module UIs are inserted with the `shiny::insertUI` function which causes such a scenario.
We already started to look for a suited solution for this problem so the problem could be removed in a future release.

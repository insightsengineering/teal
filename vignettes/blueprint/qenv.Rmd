...
In a `teal` application, a `qenv` object is created by the module developer in the module's server function. All data operations are performed within the `qenv`'s environment and their R code is stored, which allows for the code to be inspected and reproduced outside the `teal` application. This includes data loading, preprocessing, filtering, transformations, and plotting, etc.

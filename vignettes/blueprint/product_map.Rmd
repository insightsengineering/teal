...
Below, you can explore the network graph for the dependencies of the `teal` packages which shows the inter-dependencies between the core teal packages.
...
teal_deps <- pkgdepends::new_pkg_deps(
...
edges <- purrr::map_df(teal_packages, function(pkg) {
  dependent_pkgs <- resolved_teal_deps |>
    filter(package == pkg) |>
    pull(deps) |>
    bind_rows() |>
    filter(ref %in% teal_packages) |>
    pull(ref)
  tibble(
    from = dependent_pkgs,
    to = pkg

...
This can be data currently in your R session, as in the example above, but also `connectors`, which describe how to "pull" remote data when the application is run.
Connectors can pull data from external sources, which may require authentication.

In the example above we call `teal_data` to convert raw datasets into `teal` specific datasets and to bind them in one `R` object.
This function can also be used to specify relationships between different datasets by using the `join_keys` argument.

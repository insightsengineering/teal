...
* `teal` modules (a set of tabs): in this case a simple module named "example teal module".
...
* Main output panel: The outputs of the module, for this example module the chosen dataset is displayed.
...
* For the `IRIS` dataset, only rows with `Petal.Length >= 3.4` and `Species = ` `setosa` or `virginica` are included thereby keeping 50 rows.
* For the `MTCARS` dataset, only rows with `cyl = 4` or `6` are included, therevy keeping 18 rows.
...
For modules which include reproducibility functionality, it often contains a "Show R Code" button which when clicked will show the code required
to re-generate the output (including any filters added on the filter panel).


## Creating your own applications

`init` is the key function to use to define your `teal` application and to use it you require two key arguments: `data` and `modules`. 
...
In the example above we use `teal_data()` to wrap up the dataset we are using. 
This function can also be used to specify relationships between different datasets. 
In order to use CDISC clinical trial data in a `teal` application the `cdisc_data` is sued instead.  
...
The `module` argument to `init` consists of a list of `teal` modules (which can be wrapped together using the function `modules`).
...
analysis modules, for examples see:

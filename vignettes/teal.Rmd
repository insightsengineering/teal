...
## Empty `shiny` input cycle

It could be needed to use `options(shiny.useragg = FALSE)` to make the `teal::init` apps more stable, till `ragg` package is not fixed. It is recommented to put this option in the `.Rprofile` file.
Please read more about the first empty `shiny` input cycle in the `teal::init` apps in the `creating-custom-modules` vignette.

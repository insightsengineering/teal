...
<!-- These images are generated using the diagrams inside `inst/design`  -->
<span>
...
* <span style="color: #BF37F3;">Application Title</span> (browser's tab title): is the title of the application.
* <span style="color: #FB6251;">Application Header and Footer</span> (the top and the bottom of the app): any content to be placed at the top and bottom of the application.
* <span style="color: #f2b73f;">Teal Modules</span> (tabs under the header): tab for each module included in the application. In this case, there are two modules named "Module 1" and "Module 2".
* <span style="color: #83cc41;">Module Content</span> (panel on the middle): the outputs of the currently active module.
* <span style="color: #3A88FE;">Filter Panel</span> (panel on the right hand side): for filtering the data to be passed into all `teal` modules. In this case, the filter panel will be initialized with a filter for the `Species` variable in the `iris` dataset.
...
## Embedding `teal` with other `shiny` applications
`teal` can be seamlessly integrated into other `shiny` applications, enabling the creation of complex interfaces by combining multiple `teal` applications with other `shiny` elements. 

### Example: Side-by-side `teal` applications
This section demonstrates how to embed two independent `teal` applications within a single `shiny` app, using the `id` argument in the `init` function to ensure distinct identification. This approach can be generalized for embedding teal into various `shiny` modules.
```{r, message=FALSE}
app1 <- init(
...
server <- function(input, output, session) {
  app1$server(input, output, session)
  app2$server(input, output, session)
}
...
### Recommended approach for module integration
For a more modular and consistent integration, similar to standard `shiny` modules, it is advisable to use `ui_teal_with_splash` and `srv_teal_with_splash`.
...
ui <- fluidPage(
  fluidRow(
    column(6, ui_teal_with_splash(id = "app1", data = teal_data())),
    column(6, ui_teal_with_splash(id = "app2", data = teal_data()))
  )
)

...
# Abstract FilteredData Class

Currently `teal` implements a filtering model that is useful to for clinical
trials data only as it requires an `ADSL` dataset and that every dataset has the
...
concrete class inheriting from `FilteredData`.


# Modularize Teal

Currently teal needs to be a top-level shiny app, i.e.
...
of the data can be improved (and abstracted). For example,
...
changing the `ui` object that is returned by `teal::init`). Since it is not
...
module one might be interested to add a regression line or a loess smoother on
...
would be nice to have global states, e.g. the `ADSL` variable with the study arm
...
We have now three proposed ways to do get reproducible code:

1. Doug's way with parsing all the involved code and turn reactive endpoints to
ordinary function call (still need to work on that pull-request). This is a
general solution that does not require the module developer to do anything.
...
ADSL <- read_bce('/opt/BIOSTAT/qa/i9999/libraries/asl.sas7bdat')
ADTTE <- read_bce('/opt/BIOSTAT/qa/i9999/libraries/ate.sas7bdat')

## Filter Panel
ADSL_f <- ADSL %>% filter(SEX == 'F')
ADTTE_filtered <- merge(ADSL_f, ADTTE, by = c("USUBJID", "STUDYID"))
...
# Filter Labels and Filter States
...
filter configurations and reset to standard filter configurations.
...
As clinical trials data is often in a vertical form is is often necessary to do
the following operation in order to get to a particular measure for each
patient:
...
It would be great to create an abstracted filter/select shiny module that provides an
...
It would be nice to extend the catalog of very general modules. Specialized

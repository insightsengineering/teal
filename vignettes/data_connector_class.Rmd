...
library(teal)
data <- rice_data(
  rice_cdisc_dataset_connector(dataname = "ADSL", path = "/path/to/ADSL"),
  rice_cdisc_dataset_connector(dataname = "ADLB", path = "/path/to/ADLB")
)
```
...
[<frame>App Setup|
  ...
  [RelationalDataApp|
     + RelationalDataset(dataname, keys, labels)..*
     + RelationalDatasetConnector(dataname, keys, labels)..*
     + RelationalData(dataname, keys, labels)..*
     + RelationalDataConnector(dataname, keys, labels)..*
     + code]
 [<frame>teal::init()]
 [ui]
 [server]
...
foo_dataset <- function(dataname, fun) {
  dc <- relational_dataset_connector(
    pull_callable = fun,
    dataname = dataname,
    keys = keys(
      primary = "x",
...
x <- teal:::DataConnection$new(open_fun = open_fun, close_fun = close_fun) # nolint
```
...
```{nomnoml class diagram, echo=FALSE, fig.width=7, fig.height=4}
  ...
  [RelationalDatasetConnector||
     ...
     + set/get_pull_callable(CallableFunction fun)
     + set/get_pull_callable_args(args)
     ...
     - character labels
     - character path
     - CallableFunction pull_callable
     ]
  
  [RelationalDataConnection||
  ...
  [RelationalDataConnector||
    ...
    - ui
    - cdisc_data
    - RelationalDataConnection connection 0..1
    - RelationalDatasetConnectors connectors 1..
    - character code
    - logical check
    - list data
  ]
  ...
  [RelationalDataApp||
      gathers datasets, uis and servers and wraps them together]

  [CallableFunction] -> [RelationalDatasetConnector]
  [CallableFunction] -> [RelationalDataConnection]

  [RelationalDataConnection] -> [RelationalDataConnector]
  [RelationalDatasetConnector] -> [RelationalDataConnector]



  [RelationalDataset] -> [RelationalData]



  [RelationalDataConnector] ..* -> [RelationalDataApp]
  [RelationalData] ..* -> [RelationalDataApp]

...
To achieve the described setup, one must use the `mapping` argument to appropriately match each `teal_module` with its respective `teal_slice`. `mapping` takes a named list where each element is named with a module's `label` and contains a vector of `teal_slice` `id`s. These elements will active in that module at startup.

...
Filter panel was always integral part of `teal` applications. By default, filter panel is included on the right side panel. Data is filtered based before passing to the `teal` modules and reproducible filter call is included in "Show R code". Technical details of filter panel are extensively described in [`teal.slice` documentation](https://insightsengineering.github.io/teal.slice/latest-tag/).

...
Decorator failures are managed by an internal `teal` mechanism called **trigger on success**, which ensures that the `data` object within the module remains intact.
...
Here we define a decorator module that will raise an error, which we will show in action later on.
...
failing_decorator <- teal_transform_module(
  ...
  server = function(id, data) {
    moduleServer(id, function(input, output, session) {
      reactive(stop("\nThis error was raised by a decorator\n"))
    })
  }
)
```
...
When writing decorators, one must adjust them the source code of the module in question.
...
`teal_transform_module` relies on the names of the variables that exist in the module server function.
Writing a universal decorator that applies to any module may well be impossible because different modules may use different variable for their output elements.
It is possible, however, to create a parametrized decorator that will take the relevant variable names as arguments.
...
Decorators are applied to a `teal` module by passing them to the `ui_args` and `server_args` arguments of the module server functions.
There they will be passed to `ui_transform_teal_data` and `srv_transform_teal_data`, respectively.
The following examples demonstrate various uses of decorators.

### Single Decoration

In the first examlpe we will apply one decoration to one output.

#### Module

This module has one output, a plot created with `ggplot2`, and it displays the reproducible code used to obtain the plot.
We can use the decorators defined above, applying each to a separate instance of the same module.
...
Code for both outputs is also dispayed.
...
Note that a named list is passed to the module constructor.
...
Note that `my_title = input$x_axis_title` together with `xlab(my_title)` is replaced by `xlab(x_axis_table)`.
```{r}
teal_transform_module(
  label = "Static decorator",
  ui = function(id) {
...
Note that with this method all decorations wil be applied ot one output.
```{r, eval=FALSE}
...
output_data <- reactive(<holds your teal_data>)

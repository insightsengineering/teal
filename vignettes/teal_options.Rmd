...
vignette: >
  %\VignetteIndexEntry{Teal Application Options}
...
# Motivation
Some `R` packages use `options` to modify their runtime behaviour. These usually specify default values for internal functions or determine particular behaviours. For example, `testthat` uses an option `testthat.progress.max_fails` to define a default number of failed expectations before the testing functions terminate execution. While some of these adjustable values can be exposed as function parameters, some are destined to remain an option. This vignette details the options available to users of `teal`, `teal.modules.general`, `teal.modules.clinical` and `teal.devel`.
...
This is the logging level threshold used in a `teal` application. A `teal` application will not emit logs below this level. Read the documentation of `teal::register_logger` for more information. Possible values: `"TRACE"`, `"INFO"`, `"WARNING"`, `"ERROR"`.
...
The value of this option is appended to the top of the code rendered when using the `Show R Code` modal button. See the documentation of `

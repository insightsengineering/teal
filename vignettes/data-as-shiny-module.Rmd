...
Using `teal_data_module` will allow dataset creation from scratch every time the user starts the application.
...
This means server functions defined in packages cannot be used directly.
...
The `within` function allows the data in the `teal_data` object contained in `teal_data_module` to be processed at runtime.

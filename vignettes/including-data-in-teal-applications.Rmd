...
The `datanames` property lists the objects stored in the `teal_data` environment that constitute datasets of interest.
...
For convenience, an empty `datanames` property is considered to mean "all objects in the container".
...
Note that specifying `datanames` in `teal_data` is optional; if the property is empty, all objects are considered eligible.

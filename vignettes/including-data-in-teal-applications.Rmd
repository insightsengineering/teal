...
While the `teal` framework has its roots in analyzing clinical data that conforms to the CDISC standard, general, non-relational data is handled just as well, and the mechanism of passing data into `teal` is virtually the same.
Moreover, modules in `teal.modules.general` are designed to work with general data.
...
Therefore, modifying the stored datasets requires the processing code be evaluated in that environment.
...
Going back to the first example, note that since `data` was created by passing data objects, the code that was used to create the data objects is unknown, so the process cannot be reproduced.
Inspecting the in the app created above reveals a note that the preprocessing code is absent.
...
```{r}
data_with_code <- teal_data(iris = iris, cars = mtcars,
                            code = "iris <- iris
                                    cars <- mtcars")
```
...
Keep in mind that this code is not executed in the `teal_data`'s environment, so one may not reproduce the environment.
Such an object is considered _unverified_ (see [`verified` property](#verified)).

If reproducibility is required, we recommend fireat creating an empty `teal_data` object, then evaluating code.
...
In addition to general non-relational data, currently `teal` supports two specialized data structures.

### `ADaM` data

The ADaM data model from the CDISC standards, specifies relationships between subject-level "parent" dataset and observation-level "child" datasets.
...
The `MultiAssayExperiment` package offers a data structure for representing and analyzing multi-omics experiments: a bioinformatics analysis approach utilizing multiple modality of high-dimensional omics data, such as DNA mutations and abundance of RNA and proteins, in the same biological specimens.
...
The `datanames` property lists the variables stored in the `teal_data` object that constitute datasets of interest.
Variables passed to `teal_data` become automatically listed in the `datanames` property of the resulting object.
Variables created in `teal_data` by evaluating code need not be data objects of interest and as such they are not automatically added to `datanames`.
`datanames` can be read or modified with the `datanames` function.
...
The `datanames` property serves as a communication bridge between the data container and modules in a `teal` application.
In `teal` all modules are called with a `datanames` argument that determines which of the variables in the `teal_data` object they are to access.
Only variables enumerated in the `datanames` property are eligible for use in modules.

Note that specifying `datanames` in `teal_data` is optional, and if the property is empty, all objects are considered eligible.
Likewise, the `datanames` argument in the module call defaults to `"all"`, which means that module will attempt to access all eligible variables in the `teal_data` object.
...
`teal_data` allows for tracking code from data creation, through data filtering, and through data analysis, so that the whole process can be reproduced.
...
Those created with data objects alone or with data objects and code together, are not verified by default, but can become verified by running the `verify` function.

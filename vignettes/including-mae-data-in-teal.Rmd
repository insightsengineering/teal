...
## Example application
The below present an example application including `MultiAssayExperiment` data.
...
The filter panel supports `MAE` data out of the box, but `teal` itself does not guarantee that any module will work with `MAE` data the same way it works with other types of data (e.g. `ADaM`) because `MAE` has a unique
structure that needs to be considered when developing a module.

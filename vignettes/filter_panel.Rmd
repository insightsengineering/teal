...
Any of filter-panel classes have a dedicated methods to set and get current filter state. 
...
- `set_filter_state`
- `remove_filter_state`
- `remove_all_filter_states`

Setting and getting filter-panel states are done through a nested list which follows the 
specific patter. Structure of the list should reflect address of `FilterState`(s) object.
By default (`data.frame`), structure will follow pattern from the code below,
with a list element for each variable from each dataset. In case of `MultiAssayExperiment` 
objects, situation looks different, because these datasets have a multiple objects inside, 
...
```{r, message=FALSE, warning=FALSE}
isolate(
  datasets$get_data(dataname = "mtcars", filtered = TRUE)
)
```

6. Get reproducible filtering call
```{r, message=FALSE, warning=FALSE}
isolate(datasets$get_call(dataname = "mtcars"))
```

Above code can be also used in the modules. In the example below filter-panel states
are changed when clicking buttons in encoding-panel.
    ...
    module(
      ...
      ui = function(id, ...) {
        ...
        teal.devel::standard_layout(
          output = teal.devel::white_small_well(verbatimTextOutput(ns("filter_state"))),
          encoding = div(
            actionButton(ns("add_species_filter"), "Add iris$Species filter"),
      ...
      server = function(input, output, session, datasets) {
        output$filter_state <- renderText(yaml::as.yaml(datasets$get_filter_state()))

        observeEvent(input$add_species_filter,
         datasets$set_filter_state(list(iris = list(Species = list(selected = c("setosa", "versicolor")))))
        )
        observeEvent(input$remove_species_filter, {
          states <- datasets$get_filter_state()
          if (!is.null(states$iris$Species)) {
            datasets$remove_filter_state(list(iris = "Species"))
          }
        })
        observeEvent(input$remove_all_filters, datasets$remove_all_filter_states())
      }
    )
  )
)

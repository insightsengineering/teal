...
custom_reporter_template$set_template()                      # TODO: set the template function
...
# Start app with
# if (interactive()) shinyApp(custom_reporter_template$ui, custom_reporter_template$server)
```
...
app_after <- init(
  ...
  modules = modules(
    example_module("Analysis 1") |> 
      after(server = function(input, output, session, data) {           # TODO: after does not work this?
        
        rdata <- reactive({
          obj <- data()
          teal_card(obj) <- c(teal_card(obj), "## Custom Analysis Summary", head(iris))
        })
        return(data)
      })
  )

...
fun$run(args = list(seed = 1, na_percentage = 0.1))
...
# set arguments
fun$set_args(args = list(ADSL = as.name("ADSL"), cached = FALSE))
...
adsl <- relational_dataset_connector(
  dataname = "ADSL", 
  pull_callable = callable_function(radsl) %>% set_args(list(cached = TRUE)), 
...
adsl <- relational_dataset_connector(
  dataname = "ADSL", 
  pull_callable = callable_code("radsl(cached = TRUE)"), 
  keys = get_cdisc_keys("ADSL"),
  label = "ADSL"
)

...
1. `TealDataset` and `TealDatasetConnector` - `TealDataset` is a class which contains physical
data in form of `data.frame` plus reproducible code. `TealDatasetConnector` is a
class which contains R call to obtain `TealDataset`.

2. `data.frame`, `TealDataset` and `TealData`- object of class `TealDataset` contains
a single `data.frame` object. `TealData` is a collection of `TealDataset` objects.
...
# initialize object
fun <- teal.data::callable_function(fun = synthetic_cdisc_dataset)
...
fun <- teal.data::callable_function(fun = function(ADSL) { # nolint
  ADSL_2 <- ADSL # nolint
  ADSL_2$new_col <- TRUE # nolint
  ADSL_2
})
...
### `TealDataset` (including `CDISCTealDataset`)

`TealDataset` object is an R6 class which keeps a `data.frame` in its `raw_data` slot. One
can create a `TealDataset` by including `data.frame` and setting data attributes. In
...
Note `cdisc_dataset` returns an object of `CDISCTealDataset` (inherited from `TealDataset`) which
...
### `TealDatasetConnector` (including `CDISCTealDatasetConnector`)

`TealDatasetConnector` contains `Callable` object to obtain a single dataset along
...
adsl_2 <- dataset_connector(
  dataname = "ADSL_2",
  pull_callable = teal.data::callable_function(fun = function(ADSL) ADSL_2 <- ADSL) %>% # nolint
...
`TealDatasetConnector` like the other delayed data objects contains  a
...
adsl_3 <- dataset_connector(
  dataname = "ADSL_3",
  pull_callable = teal.data::callable_function(fun = function(ADSL, n) ADSL_3 <- head(ADSL, n)) %>% # nolint
    set_args(list(ADSL = as.name("ADSL"))),
  keys = teal.data::get_cdisc_keys("ADSL"),
  label = "Example label",
  vars = list(ADSL = ADSL)
...
`TealDataConnection` opens and closes connections by calling `CallableFunction` with
appropriate arguments. For example in a simplified version of `rice_connection`
below, a `TealDataConnection `uses `rice_session_open` and `rice_session_close`
functions.
...
open_fun <- teal.data::callable_function("rice::rice_session_open")
close_fun <- teal.data::callable_function("rice::rice_session_close")
close_fun$set_args(list(message = FALSE))

rice_conn <- teal:::TealDataConnection$new(
  open_fun = open_fun,
  close_fun = close_fun
)
```
...
functions respectively. As in `TealDatasetConnector$pull()`, `open()` and `close()`
...
that is why `TealDataConnection` contains a module. Developers can customize UI inputs to
...
### `TealDataConnector` (including `CDISCTealDataConnector`)
...
`TealDatasetConnector` objects and a `TealDataConnection` and we combine them
together in a `TealDataConnector` object.
...
adsl_3 <- dataset_connector(
  dataname = "ADSL_3",
  pull_callable = teal.data::callable_function(fun = function(ADSL, name, n = 5) ADSL_3 <- head(ADSL, n)) %>% # nolint
    set_args(list(ADSL = as.name("ADSL"))),
  keys = teal.data::get_cdisc_keys("ADSL"),
  label = "Example label",
  vars = list(ADSL = adsl)
...
# create connection
scda_open_fun <- teal.data::callable_function(fun = library)
scda_open_fun$set_args(list(package = "scda"))
scda_conn <- teal:::TealDataConnection$new(open_fun = scda_open_fun) # nolint

# create TealDataConnector
data <- teal:::TealDataConnector$new(
...
any close connection code. Please note that `TealDataConnector` doesn't limit
...
By default `TealDataConnector` creates simple shiny module without any
...
button is clicked. Although see `set_preopen_server` of `TealDataConnection` object for a slight
...
### `TealData` (including `CDISCTealData`)

 `TealData` manages `TealDataset`, `TealDatasetConnector` and `TealDataConnector`
objects. `CDISCTealData` is the equivalent object when creating apps to analyse CDISC data.
...
include them in a shiny application. `TealData` gathers all objects and sets
...
them in the same way how we link `TealDatasetConnector` objects. If object code

...
app <- teal::init(data = teal_data(dataset("IRIS", x = iris)),
...
```{r, eval=TRUE}
datasets$set_bookmark_state(state = list(ADSL = list(USUBJID = sprintf("id-%s", 1:4))))
  ...
  init(
    ...
    modules = root_modules(
      module(
        ...
        ui = function(id, ...)
          div(p("information about data source")),
        filters = "all"
      ),
      module(
        "ADSL AGE histogram",
...
shinyApp(
  ui = fluidPage(fluidRow(column(6, app1$ui),
                          column(6, app2$ui))),
  server = function(input, output, session) {

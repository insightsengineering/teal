...
datasets$set_dataset(dataset = cdisc_dataset("ADSL", ADSL))
datasets$set_dataset(dataset = cdisc_dataset("ADTTE", ADTTE),
...
```{r}
cat(
  get_filter_expr(datasets, "ADTTE")
)
```
...
filter variables if we analyse an output based on `ADTE`).
...
want to use the teal app in the same way as Shiny modules with `callModule`.

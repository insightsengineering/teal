...
x <- teal::init(
  data = list(ADSL = iris %>% dplyr::mutate(USUBJID = 1:n(), STUDYID = "A")),
  modules = root_modules(
    module(
      label = "sample module",
      server = function(input, output, session, datasets) {
        output$scatterplot <- renderPlot({
          iris <- datasets$get_data("ADSL", filtered = TRUE, reactive = TRUE)
...
This fitering will now be explained step by step for two example datasets (`ADSL`
and `ATE`):
...
```{r}
options(teal_logging = FALSE)
datasets <- teal:::FilteredData$new(datanames = c("ADSL", "ADTTE"))
...
```{r}
datasets$set_filter_state("ADSL", "USUBJID", state = paste0("id-", 1:4))

datasets$get_data("ADSL", reactive = FALSE, filtered = TRUE)
```
...
```{r}
datasets$get_data("ADTTE", reactive = FALSE, filtered = TRUE)
```
...
```{r}
datasets$get_filter_call("ADTTE", merge = TRUE, adsl = TRUE)
```

...
The `teal` package offers an integrated reporting feature utilizing the `vignette("teal.reporter")` package. For a more comprehensive understanding of the reporting functionality itself, please refer to the official documentation.
...
The entire lifecycle of objects involved in creating the report and configuring the module to preview the report is efficiently handled by `teal`.

## Custom module

Let's consider the example module from `teal`:
```{r, message=FALSE}
library(teal)
...
Using `teal`, you can launch this example module with the following lines:
...
The `teal` platform adds another tab to the application titled `Report previewer`. However, it seems that there is no visible change in how the module operates and appears. As of now, users cannot interact with it to add content to the report from this module. Fortunately, `teal.reporter` provides `ui` and `server` objects to facilitate this.

### Introduce the new `UI` and supporting `shiny` modules

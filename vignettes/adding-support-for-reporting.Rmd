...
- Adding support for reporting to their module, by returning a reactive `teal_data` object from the `server` function of the module.
  ...
  module(
    ...
    server = function(id, data, reporter) {
      moduleServer(id, function(input, output, session) {
        ...
        rdata <- reactive({
          teal_card(data()) <- teal_card(data(), "## Example heading", head(iris))
        })
        return(rdata)
      })
    },
    ...
    server = function(input, output, session, data, reporter, ...) {
      ...
      report_data <- reactive({
        ...
        teal.reporter::teal_card(obj) <- 
          c(teal.reporter::teal_card(obj), "# Module's computation")
        eval_code(
          obj, 
          sprintf(

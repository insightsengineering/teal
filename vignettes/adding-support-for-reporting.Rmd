    ...
    server = function(id, datasets, reporter) {
      moduleServer(id, function(input, output, session) {
        ...
        table_r <- reactive({
          ...
          teal.code::chunks_push(bquote(result <- .(dput(dat()))), id = "get_data", chunks = simple_chunks)
        ...
        card_fun <- function(card = TealReportCard$new(), comment) {
          ...
          card$append_text("Encoding", "header2")
          ...
          card$append_text("Show R Code", "header2")
          card$append_src(paste(teal.code::chunks_get_rcode(simple_chunks), collapse = "\n"))
          if (!comment == "") {
...
app <- init(
  ...
  modules = modules(
    example_reporter_module("with Reporter"),
    example_module(lable = "without Reporter")
  ),

...
## Removing support for Show R code

If your module supports a report but you want to disable the show R code functionality `disable_src()`
...
One can change the template so that it has some content by default.
We can also add a loaded report. 

```{r reporter_6}
# 1. Reporter with a pre-defined template that gets applied to every future report card
reporter <- teal.reporter::Reporter$new()
template_fun <- function(document) {
  header <- teal.reporter::teal_card("Here comes header text.") 
  footer <- teal.reporter::teal_card("Here comes footer text. Report generated with teal ![logo](https://raw.githubusercontent.com/insightsengineering/teal/refs/heads/main/man/figures/logo.svg 'teal logo'){height=70}")
  c(header, document, footer)

}
reporter$set_template(template_fun)

# 2. Reporter with a pre-defined card
card1 <- teal.reporter::teal_card("## Header 2 text", "Regular text")
metadata(card1, "title") <- "Welcome card"
reporter$append_cards(card1)
```

Once the reporter is added we can use in the teal application.
...
app <- init(
  ...
  modules = modules(
    # 3. Module with an edited report (added by app developer)
    example_module(),
    example_module(label = "extend reporter") |>  
      after(server = function(data) {
        teal.reporter::teal_card(data) <- 
          c(teal.reporter::teal_card(data), teal.reporter::teal_card("## New title", "text"))
        data
      })
  ),
  reporter = reporter

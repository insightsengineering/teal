...
test_that("One cached and one dependent connector wrapped in a single rcd data connector", {
  adsl <- rcd_cdisc_dataset_connector("ADSL", radsl, seed = 1)
  adae <- rcd_cdisc_dataset_connector("ADAE", radae, ADSL = adsl, seed = 1)
  ...
  expect_equal(items$ADSL$get_pull_callable()$get_call(), "radsl(seed = 1)")
  expect_equal(items$ADAE$get_pull_callable()$get_call(), "radae(ADSL = ADSL, seed = 1)")

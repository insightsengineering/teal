...
testthat::test_that("get_call returns a call filtering a data.frame based on a DatetimeFilterState", {
  datetime_dataset <- data.frame(
    datetime = seq(ISOdate(2021, 8, 25), by = "day", length.out = 3)
  )
  filter_states <- FilterStates$new(
  ...
  filter_states$queue_initialize(list(ReactiveQueue$new()))
  datetime_filter <- DatetimeFilterState$new(x = ISOdate(2021, 8, 27), varname = "datetime")
  ...
  testthat::expect_equal(
    ...
    bquote(
      datetime_output <- subset(
        datetime_dataset,
        datetime >= as.POSIXct("2021-08-27 12:00:00", tz = "GMT") &
          datetime < as.POSIXct("2021-08-27 12:00:01", tz = "GMT")
      )
    )
  )
})

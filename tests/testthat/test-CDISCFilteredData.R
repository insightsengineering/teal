...
isolate({
  ...
  test_that("set filter state - single var", {
    ...
    expect_identical(
      trimws(deparse(ds$get_filter_expr(dataname = "ADSL"), width.cutoff = 500L)[expected_lines]),
      c("{", "ADSL_FILTERED <- dplyr::filter(ADSL, is.na(AGE) | (is.infinite(AGE) | AGE >= 30 & AGE <= 50))", "}")
    )
  ...
  test_that("set filter state - multiple vars", {
    ...
    expected_lines <- covr_deparse_fix(c(1, 4, 6), c(1, 2, 3))
    expect_identical(
      trimws(deparse(ds$get_filter_expr("ADSL"), width.cutoff = 500L)[expected_lines]),
      c("{", "ADSL_FILTERED <- dplyr::filter(ADSL, AGE >= 21 & AGE <= 68, SEX %in% c(\"M\", \"F\"))", "}")
    )
  ...
  test_that("set_filter_state works with more than one filter specified", {
    ...
    expect_identical(
      trimws(deparse(ds$get_filter_expr("ADSL"), width.cutoff = 500L)[expected_lines]),
      c("{", "ADSL_FILTERED <- dplyr::filter(ADSL, is.na(AGE) | AGE >= 38 & AGE <= 40, is.na(SEX) | SEX == \"F\")", "}") #nolintr
    )
  ...
  test_that("keep_na and keep_inf", {
    ...
    expect_identical(
      trimws(deparse(ds$get_filter_expr("ADSL"), width.cutoff = 500L)[expected_lines]),
      c("{", "ADSL_FILTERED <- dplyr::filter(ADSL, is.infinite(AGE) | AGE >= 20 & AGE <= 69, is.na(SEX) | SEX == \"F\")", "}") # nolint
    )
    ...
    expect_identical(
      trimws(deparse(ds$get_filter_expr(dataname = "ADSL"), width.cutoff = 500L)[expected_lines]),
      c("{", "ADSL_FILTERED <- dplyr::filter(ADSL, is.na(AGE) | AGE >= 38 & AGE <= 40, SEX == \"F\", COUNTRY %in% c(\"CHN\", \"USA\", \"BRA\"))", "}") # nolint
    )
  ...
  test_that("non ADSL filter", {
    ...
    expect_identical(
      trimws(deparse(ds$get_filter_expr("ADSL"), width.cutoff = 500L)[expected_lines]),
      c("{", "ADSL_FILTERED <- dplyr::filter(ADSL, is.na(AGE) | AGE >= 38 & AGE <= 40, SEX == \"F\")", "}")
    )

    expected_lines <- covr_deparse_fix(c(4, 8), c(2, 3))
    expect_identical(
      ...
      c(
        ...
        "ADAE_FILTERED <- dplyr::inner_join(x = ADAE_FILTERED_ALONE, y = ADSL_FILTERED[, c(\"STUDYID\", \"USUBJID\"), drop = FALSE], by = c(\"STUDYID\", \"USUBJID\"))" # nolint
      )
    )
  })
  ...
  test_that("column_labels works", {
    ...
    expect_equal(
      ds$get_varlabels("ADSL2"),
      NULL
    )
    # only some variables
    expect_equal(
      ds$get_varlabels("ADSL2", variables = c("AGE", "SEX")),
      NULL
    )
  })
})

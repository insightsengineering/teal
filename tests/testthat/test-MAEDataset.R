...
testthat::test_that("MAEDataset$check returns TRUE when constructed with the correct code", {
  exprss1 <- matrix(
...
testthat::test_that("FALSE returned when executing MAEDataset$check and code is not correct", {
  exprss1 <- matrix(
    seq(from = 1, by = 0.1, length.out = 16),
    ncol = 4,
    dimnames = list(sprintf("ENST00000%i", seq.int(1, 4)), c("Jack", "Jill", "Bob", "Bobby"))
...
testthat::test_that("Error raised when executing MAEDataset$check and code is empty", {
  exprss1 <- matrix(
    seq(from = 1, by = 0.1, length.out = 16),
    ncol = 4,
    dimnames = list(sprintf("ENST00000%i", seq.int(1, 4)), c("Jack", "Jill", "Bob", "Bobby"))
  ...
  mae_dataset <- MAEDataset$new(dataname = "simple_mae", x = simple_mae, code = "")
...
testthat::test_that("Error raised executing MAEDataset$check_keys and duplicate rows found in key columns", {
  array_data <- matrix(
    seq(101, 108),
    ncol = 4,
  ...
  mae_dataset <- MAEDataset$new(dataname = "test_mae", x = test_mae, keys = "sample_id")
  testthat::expect_error(
...
testthat::test_that("mae_dataset() constructor returns the same as MAEDataset$new()", {
  mae1 <- mae_dataset("mae", MultiAssayExperiment::miniACC)
  mae2 <- MAEDataset$new("mae", MultiAssayExperiment::miniACC)
  testthat::expect_equal(mae1, mae2)
})

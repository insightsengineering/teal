context("columns_spec")

test_that("Proper argument types", {
  choices <- c("c1", "c2", "c3")
  selected <- c("c1", "c2")
  expect_silent(columns_spec(choices = choices, selected = selected))

  expect_error(columns_spec(choices = list(list(choices)), selected = selected))
  expect_error(columns_spec(choices = choices, selected = list(list(selected))))
  expect_error(columns_spec(choices = choices, selected = selected, multiple = 1), "is.logical.single")
  expect_error(columns_spec(choices = choices, selected = selected, multiple = c(TRUE, TRUE)), "is.logical.single")
  expect_error(columns_spec(choices = choices, selected = selected, fixed = 1), "is.logical.single")
  expect_error(columns_spec(choices = choices, selected = selected, label = factor("Hello")), "is.character.single")

  #
  choices <- list(c("a", "b"), c("c", "d"))
  expect_error(columns_spec(choices = choices, selected = c("a", "b")), "selected %is_in%")
  expect_error(columns_spec(choices = choices, selected = list(c("a", "c"))), "selected %is_in%")
  expect_silent(columns_spec(choices = choices, selected = list(c("a", "b"))))
  expect_silent(columns_spec(choices = c("a", "b", "c"), selected = c("a", "b")))

})

test_that("Single choice", {
  expect_silent(
    c1 <- columns_spec(choices = c("AVAL", "BMRKR1", "AGE"),
                       selected = c("AVAL"),
                       fixed = FALSE,
                       label = "Column")
  )
  expect_silent(
    c2 <- columns_spec(choices = c("AVAL", "BMRKR1", "AGE"),
                       fixed = FALSE,
                       label = "Column")
  )

  expect_identical(c1, c2)
  expect_identical(class(c1), "column_spec")
  ...
  # minimal example
  expect_silent(c3 <- columns_spec(choices = c("AVAL", "BMRKR1", "AGE")))
  expect_identical(class(c3), "column_spec")
...
test_that("Multiple choices", {
  ...
  expect_silent(c3 <- columns_spec(choices = choices, selected = selected))

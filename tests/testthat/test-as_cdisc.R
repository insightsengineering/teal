testthat::test_that("as_cdisc does not modify a cdisc-flavoured object", {
  ds <- teal.data::cdisc_dataset("ADSL", as.data.frame(setNames(nm = teal.data::get_cdisc_keys("ADSL"))))
  testthat::expect_identical(as_cdisc(ds), ds)
  conn <- cdisc_dataset_connector(
    "ADSL",
    callable_code("as.data.frame(setNames(nm = teal.data::get_cdisc_keys(\"ADSL\")))"),
...
testthat::test_that("as_cdisc passes keys from dataset to cdisc dataset", {
  ds <- teal.data::dataset(
    "ADSL",
    as.data.frame(setNames(nm = c(teal.data::get_cdisc_keys("ADSL"), "SUBJID"))),
...
testthat::test_that("as_cdisc assigns cdisc keys if dataset has no keys and name matches cdisc", {
  ds <- teal.data::dataset("ADSL", as.data.frame(setNames(nm = teal.data::get_cdisc_keys("ADSL"))))
  testthat::expect_length(get_keys(ds), 0)
  testthat::expect_equal(get_keys(as_cdisc(ds)), teal.data::get_cdisc_keys("ADSL"))
})

testthat::test_that("as_cdisc does not assign cdisc keys if dataset has no keys, but name does not match cdisc", {
  ds <- teal.data::dataset("test", as.data.frame(setNames(nm = teal.data::get_cdisc_keys("ADSL"))))
  testthat::expect_length(get_keys(ds), 0)

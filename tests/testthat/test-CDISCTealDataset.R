...
testthat::test_that("Case 1: CDISCTealDataset$get_code() does not return duplicated code when
  CDISCTealDataset$mutate method is called", {
  iris_dataset <- CDISCTealDataset$new("iris", head(iris), code = "head(iris)", parent = character(0), keys = c("test"))
  mtcars_dataset <- CDISCTealDatasetConnector$new(
    "mtcars",

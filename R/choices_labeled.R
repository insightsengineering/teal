...
#' choices1 <- choices_labeled(names(ADSL), get_variable_labels(ADSL, fill = FALSE))
#' choices2 <- choices_labeled(ADTTE$PARAMCD, ADTTE$PARAM)
#' # if only a subset of variables are needed, use subset argument
#' choices3 <- choices_labeled(names(ADSL), get_variable_labels(ADSL, fill = FALSE), subset = c("ARMCD", "ARM"))
#' \dontrun{
#' shinyApp(
#'   ui = fluidPage(
#'     selectInput("c1",
...
variable_choices.data.frame <- function(data, subset = NULL, fill = FALSE, key = NULL) { # nolint
  ...
  res <- if ("" %in% subset) {
    choices_labeled(
      ...
      labels = c("", unname(get_variable_labels(data, fill = fill))),
      subset = subset,
      types = c("", var_types)
    )
  } else {
    choices_labeled(
      choices = names(data),
      labels = unname(get_variable_labels(data, fill = fill)),
      subset = subset,
      types = var_types
    )
  }

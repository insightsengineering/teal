...
#'
#' This module creates two objects called `object` and `object2` that can be modified with decorators.
#' The `object` is determined by what's selected in `Choose a dataset` input in UI.
...
#' }
#' @export
example_module2 <- function(label = "example teal module with two outputs",
                           datanames = "all",
                           transformators = list(),
                           decorators = list()) {
  checkmate::assert_string(label)

  ...
  public = list(
    ...
    eval = function(envir = new.env(parent = parent.env(.GlobalEnv))) {
     for (x in self$get_code(deparse = FALSE)) {
       ...
       if (is(out, "error")) {
         error_msg <- sprintf("%s\n\nEvaluation of the code failed:\n %s", deparse1(x), conditionMessage(out))
    ...
    for (var_idx in seq_along(x)) {
      ...
      if (is(var_value, "DatasetConnector") || is(var_value, "Dataset")) {
        ...
        if (var_name != var_value$get_dataname()) {
          res$set_code(
            deparse1(call("<-", as.name(var_name), as.name(var_value$get_dataname()))),
            dataname = var_value$get_dataname()
          )
        }
      } else {
        var_code <- deparse1(call("<-", as.name(var_name), var_value))
  ...
  ulapply(
    ...
    function(code_single) {
      if (is_empty(parse(text = code_single))) {
        ...
        vapply(as.list(as.call(parse(text = code_single))), deparse1, character(1))
      }
    }
  )
}
